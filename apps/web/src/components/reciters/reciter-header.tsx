...
export function ReciterHeader({ reciter }: ReciterHeaderProps): React.JSX.Element {
  ...
  return (
    <div className="flex flex-col items-center gap-4 py-8 text-center sm:flex-row sm:items-start sm:text-left">
      ...
      <div
        aria-hidden="true"
        className="flex h-24 w-24 shrink-0 items-center justify-center rounded-full bg-gray-200 text-3xl font-semibold text-gray-700"
      >
        {initials}
      </div>

      <div className="flex flex-col gap-1">
        <h1 className="text-3xl font-bold text-gray-900">{reciter.name}</h1>
